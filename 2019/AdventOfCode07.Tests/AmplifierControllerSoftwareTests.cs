...
namespace AdventOfCode07.Tests
{
    public class AmplifierControllerSoftwareTests
    {
        ...
        public void TestExamplesFeedbackLoop(string program, long expectedMaxThrusterSignal)
        {
            var controller = new FeedbackLoopedAmplifierController(program);

            Assert.Equal(expectedMaxThrusterSignal, controller.DetermineMaxThrusterSignal());
        }

        [Fact]
        public void TestProgramFeedbackLoop()
        {
            var program = File.ReadAllText("Program.txt");

            var controller = new FeedbackLoopedAmplifierController(program);

            Assert.Equal(39016654, controller.DetermineMaxThrusterSignal());
        }
    }
}
